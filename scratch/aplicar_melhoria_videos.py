...
# Definir o novo bloco de código que substituirá o antigo
...
print(f"Arquivos mapeados para atualizar: {arquivos}")
...
for arq in arquivos:
    try:
        ...
        idx_ativos = code.find('def _extrair_dados_ativos')
        idx_vincular = code.find('def _vincular_material_ao_modulo')

        if idx_ativos != -1 and idx_vincular != -1:
            # Pegar a indentação da palavra 'def _extrair_dados_ativos'
            # Vamos achar o início da linha
            inicio_linha = code.rfind('\n', 0, idx_ativos) + 1
            indentacao = code[inicio_linha:idx_ativos]
            
            # Ajustar o novo bloco de código para a mesma indentação se necessário
            # (já está com 4 espaços de indentação padrão)
            
            codigo_novo = code[:idx_ativos] + novo_bloco + '\n' + code[idx_vincular:]
            
            with open(arq, 'w', encoding='utf-8') as f:
                f.write(codigo_novo)
            print(f"[OK] Atualizado com sucesso: {arq}")
        else:
            print(f"[ERRO] Métodos não localizados em {arq}: idx_ativos={idx_ativos}, idx_vincular={idx_vincular}")
