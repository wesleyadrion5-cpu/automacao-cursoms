...
DIAGNOSTICS_DIR = os.path.join(BASE_DIR, "logs", "diagnosticos")
...
class FrotaWorker(threading.Thread):
    ...
    def _sessao_ativa(self):
        try:
            return bool(self._driver and self._driver.session_id and self.driver.window_handles)
        except Exception:
            return False
    ...
    def _trocar_para_janela_segura(self, handle, contexto):
        if not self._sessao_ativa():
            raise RuntimeError(f"Sessão do navegador indisponível ao tentar acessar {contexto}.")
        handles = list(self.driver.window_handles)
        if handle not in handles:
            raise RuntimeError(
                f"Janela indisponível ao tentar acessar {contexto}. Handles atuais: {len(handles)}"
            )
    ...
    def _pesquisar_modulo_novo(self, wait, nome_modulo, tentativas=2, intervalo_base=2.5):
        ultimo_erro = None
        for tentativa in range(tentativas):
            try:
                search = self._abrir_lista_modulos_novo(wait)
                self.preencher_input(search, nome_modulo)
                time.sleep(intervalo_base + tentativa)
                modulo = self._encontrar_modulo_no_novo(nome_modulo)
                if modulo:
                    return modulo
        ...
        if ultimo_erro:
            self.log(
                "INFO",
                f"Pesquisa do módulo '{nome_modulo}' não confirmou resultado: {str(ultimo_erro)[:70]}",
            )
        return None

    def _buscar_url_aulas_modulo_novo(self, wait, nome_modulo, tentativas=4):
        for tentativa in range(tentativas):
            modulo = self._pesquisar_modulo_novo(
                wait, nome_modulo, tentativas=1, intervalo_base=2.5 + tentativa
            )
            if modulo:
                try:
                    tr = modulo.find_element(By.XPATH, "./ancestor::tr")
                    return tr.find_element(By.XPATH, NOVO_LESSONS_LINK_XPATH).get_attribute("href")
                except Exception:
                    time.sleep(1)
            time.sleep(1.5)
        return None
    ...
    def run(self):
        try:
            ...
            self._trocar_para_janela_segura(aba_nova, "login do site novo")
            ...
            self._trocar_para_janela_segura(aba_antiga, "retorno ao site antigo após login")
            ...
            self._trocar_para_janela_segura(aba_antiga, "captura da URL do curso antigo")
            ...
            for row in self.chunk_excel:
                ...
                try:
                    ...
                    modulo_existente = self._pesquisar_modulo_novo(wait, nome_modulo, tentativas=2)
                    if modulo_existente:
                        self.log(
                            "INFO",
                ...
                for tentativa in range(2):
                    ...
                    try:
                        ...
                        self.preencher_input(
                            wait.until(
                                EC.presence_of_element_located((By.XPATH, NOVO_MODULE_TIME_XPATH))
                            ),
                            "0",
                        )
                        ...
                        if self._erro_sessao_invalida(e):
                            self.log(
                                "ERRO",
                                "Sessão do Chrome perdida. Encerrando este worker para evitar erros em cascata.",
                            )
                ...
                for url in urls:
                    ...
                    try:
                        ...
                        for al in set(self.driver.window_handles) - ja:
                            self._trocar_para_janela_segura(al, "fechamento de aba temporária de vídeo")
                            self.driver.close()
                ...
                try:
                    for ab in self.driver.window_handles:
                        if ab != aba_ant and ab != aba_nov:
                            self._trocar_para_janela_segura(ab, "fechamento de abas temporárias de material")
                            self.driver.close()
