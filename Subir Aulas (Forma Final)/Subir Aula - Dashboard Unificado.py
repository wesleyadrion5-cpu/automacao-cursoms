...
if IS_FROZEN:
    # No executável compilado (.exe):
    # Recursos estáticos imutáveis (como a música e ícones) são extraídos no diretório temporário
    RESOURCES_DIR = sys._MEIPASS
    # Arquivos mutáveis e de escrita do usuário (banco de dados, configs, logs) devem ficar
    # no mesmo diretório do arquivo .exe para persistência permanente de dados
    APP_DIR = os.path.dirname(sys.executable)
    
    BASE_DIR = APP_DIR
    PARENT_DIR = APP_DIR
    CONFIG_PATH = os.path.join(APP_DIR, "config_unificada.json")
    LOGS_DIR = os.path.join(APP_DIR, "logs")
    DIAGNOSTICS_DIR = os.path.join(APP_DIR, "logs", "diagnosticos")
    DB_PATH = os.path.join(APP_DIR, "banco_frota.db")
else:
    # Em modo de desenvolvimento (rodando o script .py direto):
    ...
    # Busca a biblioteca refatorada no diretório pai, onde ela está de fato localizada
    REFATORACAO_SRC_DIR = os.path.join(PARENT_DIR, "refatoracao_cursoms", "src")
...
try:
    ...
    def load_excel_records(path):
        ...
        if not set(df.columns).intersection(
            {"nome do modulo", "nome do módulo", "modulo", "módulo"}
        ):
            raise ValueError(f"A planilha '{os.path.basename(path)}' não possui coluna de módulo.")
    ...
    def bypass_chrome_certificate_warning(driver, context="", timeout=4):
        ...
        while time.time() < fim:
            ...
            acao = ""
            try:
                acao = str(driver.execute_script("""
...
class FrotaWorker(threading.Thread):
    ...
    def calcular_similaridade(self, nome_excel, nome_site):
        ...
        # Remove sufixos de concurso: (Concurso...), (Prefeitura...), etc.
        n_exc = re.sub(r"\(pacatuba/ce\)", "", n_exc)
        n_exc = re.sub(r"\(concurso[^)]*\)", "", n_exc).strip()
        n_exc = re.sub(r"\(prefeitura[^)]*\)", "", n_exc).strip()
        n_exc = re.sub(r"\([^)]{10,}\)$", "", n_exc).strip()  # qualquer parêntese longo no final
    ...
    def _tentar_recuperar_overlay_500(self, contexto="", tentativas=3):
        ...
        self.log(
            "INFO", f"Overlay 500 detectado{detalhe}. Tentando clicar fora para liberar a tela."
        )
    ...
    def _buscar_url_aulas_modulo_novo(
        self, wait, nome_modulo, professor="", curso="", tentativas=5
    ):
        for tentativa in range(tentativas):
            ...
            try:
                ...
                for h6 in self.driver.find_elements(By.TAG_NAME, "h6"):
                    ...
                    try:
                        ...
                        href = (
                            tr.find_element(By.XPATH, NOVO_LESSONS_LINK_XPATH).get_attribute("href")
                            or ""
                        )
                        if not href:
                            continue
                ...
                destino = (
                    melhor_url
                    or destino
                    or self._buscar_url_aulas_modulo_novo(
                        wait,
                        nome_modulo,
                        professor=professor,
                        curso=curso,
    ...
    def _selecao_livewire_confirmada(self, container, input_elem, texto_pesquisa):
        ...
        try:
            for botao in container.find_elements(
                By.XPATH, ".//*[@wire:click='clearSelectedValue']"
            ):
        ...
        return bool(
            valor_atual
            and (
                valor_atual == texto_norm or texto_norm in valor_atual or valor_atual in texto_norm
            )
        )
        ...
        try:
            ...
            for row in self.chunk_excel:
                ...
                try:
                    ...
                    url_aulas_existente = (
                        str(modulo_existente.get("url_aulas", "") or "").strip()
                        if modulo_existente
                        else ""
                    )
                ...
                for tentativa in range(2):
                    ...
                    try:
                        ...
                        # 1. Prioridade máxima: verificar a última coluna da linha do Excel (ignorando campos internos que começam com '_')
                        if row:
                            chaves_colunas = [k for k in row.keys() if not k.startswith("_")]
                            if chaves_colunas:
                                ultima_coluna = chaves_colunas[-1]
                                valor_ultima = str(row[ultima_coluna]).strip()
                                if valor_ultima.startswith("http://") or valor_ultima.startswith("https://"):
                                    link_modulo_direto = valor_ultima
                        
                        # 2. Fallback para colunas com nomes específicos de link
                        if not link_modulo_direto:
                            for col_link in ["link", "url", "link do modulo", "url do modulo", "link modulo", "url modulo", "link antigo", "link do modulo antigo"]:
                                if col_link in row:
                                    val = str(row[col_link]).strip()
                                    if val.startswith("http://") or val.startswith("https://"):
                                        link_modulo_direto = val
                                        break
                        
                        # 3. Fallback genérico varrendo todas as colunas em ordem reversa
                        if not link_modulo_direto:
                            for k, v in reversed(list(row.items())):
                                if k.startswith("_"):
                                    continue
                                v_str = str(v).strip()
                                if v_str.startswith("http://") or v_str.startswith("https://"):
                                    if "cursoms.com.br" in v_str and "aula.asp" in v_str:
                                        link_modulo_direto = v_str
                                        break
                                    elif "novo.cursoms" not in v_str and "google" not in v_str:
                                        link_modulo_direto = v_str
                                        break

                        modulo_localizado = False

                        if link_modulo_direto:
                            self.log("INFO", f"Navegando direto para o módulo antigo pelo link fornecido: {link_modulo_direto}")
                            self._abrir_url_com_certificado(link_modulo_direto, f"detalhes do módulo antigo (link direto): {nome_modulo}")
                            time.sleep(1.5)
                            modulo_localizado = True
                        else:
                            self._abrir_url_com_certificado(
                                url_lista_antiga, "lista de módulos antigos"
                            )
                            time.sleep(1)
                            tds = wait.until(
                            ...
                            if metodo_busca == "Ordem Exata (Linha 1 = Módulo 1)":
                                if indice_origem < len(tds):
                                    ...
                                    self.log(
                                        "ERRO",
                                        "O site antigo tem menos módulos do que o seu Excel.",
                                    )
                            else:
                                # MODO FUZZY MATCH — busca pelo nome mais parecido na lista da plataforma antiga
                                for td in tds:
                            ...
                            if melhor_td and maior_nota >= limite_rigor:
                                nome_encontrado = melhor_td.text.strip()
                                self.log(
                                    "INFO",
                                    f"🎯 MÓDULO ENCONTRADO NO ANTIGO | Score: {maior_nota:.2f}\n"
                                    f"   Planilha: '{nome_modulo}'\n"
                                    f"   Site:     '{nome_encontrado}'",
                                )
                        ...
                        if (
                            modo_somente_conteudo
                            and not self.fila_videos
                            and not self.fila_arquivos
                        ):
                            self.log(
                                "INFO",
                        ...
                        WebDriverWait(self.driver, 8).until(
                            ...
                            or len(d.find_elements(By.CSS_SELECTOR, SELECTORS.modules_search_css))
                            > 0
    ...
    def _localizar_elemento_recursivo_frames(self, xpath):
        ...
        def _buscar():
            ...
            frames = self.driver.find_elements(By.TAG_NAME, "frame") + self.driver.find_elements(
                By.TAG_NAME, "iframe"
            )
            for f in frames:
                try:
    ...
    def _extrair_dados_ativos(self, capturar_videos=True, capturar_materiais=True):
        ...
        if capturar_videos:
            ...
            botao_video = self._localizar_elemento_recursivo_frames(
                "//a[contains(@href, 'videos.asp')]"
            )
            if botao_video:
                ...
                if (
                    href_videos
                    and not href_videos.startswith("#")
                    and "javascript" not in href_videos
                ):
                ...
                def buscar_todos_links(lista_acumulada):
                    lk = self.driver.find_elements(
                        By.XPATH, "//a[contains(@href, 'alterar_video.asp')]"
                    )
                    for l in lk:
                ...
                if urls:
                    ...
                    for idx, url in enumerate(urls, 1):
                        ...
                        try:
                            ...
                            na = open_new_tab(
                                self.driver, url, "extração de vídeo em nova aba", timeout=5
                            )
                            self._trocar_para_janela_segura(na, "extração de vídeo em nova aba")
                            self._ignorar_aviso_certificado_se_aparecer(
                                "extração de vídeo em nova aba"
                            )
                            ...
                            self.log(
                                "OK",
                                f"Vídeo {idx} extraído: '{tit}' -> Vimeo: {dados_video.get('vimeo')}, Youtube: {dados_video.get('youtube_link')}",
                            )
                            ...
                            self._trocar_para_janela_segura(
                                jp, "retorno após falha na extração de vídeo"
                            )
                else:
                    self.log(
                        "AVISO",
                        "Nenhum link de vídeo (alterar_video.asp) encontrado na página de listagem!",
                    )
            else:
                self.log(
                    "AVISO", "Botão de vídeos (videos.asp) NÃO encontrado na página do módulo!"
                )
            ...
            for nm, inf in setores.items():
                ...
                if botao_setor:
                    try:
                        ...
                        if (
                            href_setor
                            and not href_setor.startswith("#")
                            and "javascript" not in href_setor
                        ):
                        ...
                        try:
                            ...
                            self.driver.execute_script(
                                ...
                                self.driver.find_element(
                                    By.CSS_SELECTOR, "span[data-number='all']"
                                ),
                            )
                            time.sleep(2.5)
                        except:
                    ...
                    self._abrir_url_com_certificado(
                        d_url, "retorno ao módulo antigo após materiais"
                    )
                else:
        ...
        if capturar_videos:
            ...
            if botao_video:
                ...
                if (
                    ...
                    self._abrir_url_com_certificado(
                        href_videos, "lista de vídeos do módulo único no site antigo"
                    )
                else:
                    self.driver.execute_script("arguments[0].click();", botao_video)
                time.sleep(2)
                ...
                self.log(
                    "INFO", "Procurando links de vídeos (alterar_video.asp) para módulo único..."
                )
                self.driver.switch_to.default_content()
                urls = []
                buscar_todos_links(urls)
                ...
                if urls:
                    ...
                    for idx, url in enumerate(urls, 1):
                        ...
                        try:
                            ...
                            self._ignorar_aviso_certificado_se_aparecer(
                                "extração de vídeo do módulo único"
                            )

                            wait_local = WebDriverWait(self.driver, 5)
                            elem_t = wait_local.until(
                ...
                self.log(
                    "AVISO",
                    "Botão de vídeos (videos.asp) NÃO encontrado na página do módulo único!",
                )
                ...
                if botao_setor:
                    try:
                        ...
                        if (
                            ...
                            self._abrir_url_com_certificado(
                                href_setor, f"setor do módulo único: {nome}"
                            )
                        else:
                            self.driver.execute_script("arguments[0].click();", botao_setor)
                        time.sleep(2)
                        ...
                        for item in soup.find_all("div", class_="list-item box"):
                            ...
                            titulo = (
                                subject.text.strip() if subject and subject.text else "Sem Titulo"
                            )
                        ...
                        if lote_setor:
                            self.log(
                                "INFO", f"Encontrados {len(lote_setor)} materiais no setor {nome}."
                            )
                            lote_setor.reverse()
                    ...
                    self._abrir_url_com_certificado(
                        dashboard_url, "retorno ao módulo único após materiais"
                    )
                else:
    ...
    def _injetar_materiais(self, wait, m_nome, aba_ant, aba_nov):
        ...
        for d in list(self.fila_arquivos):
            ...
            try:
                ...
                lk = bs.find("a", href=lambda h: any(ext in str(h).lower() for ext in exts))
                if not lk:
                ...
                self.driver.get("https://novo.cursoms.com.br/attachments/create")
                    ...
                    EC.presence_of_element_located((By.CSS_SELECTOR, SELECTORS.attachment_file_css))
                )
                self.driver.execute_script(
                ...
                i_n = wait.until(
                    EC.element_to_be_clickable((By.CSS_SELECTOR, SELECTORS.attachment_name_css))
                )
...
class AppPrincipal(ctk.CTk):
    def __init__(self):
        ...
        # Configurar o ícone de janela
        if IS_FROZEN:
            icon_path = os.path.join(RESOURCES_DIR, "Play Branco.ico")
        else:
            icon_path = os.path.join(PARENT_DIR, "Play Branco.ico")
            
        if not os.path.exists(icon_path):
            icon_path = os.path.join(BASE_DIR, "Play Branco.ico")
    ...
    def carregar_dados_login(self):
        ...
        try:
            ...
            print(f"[Supabase] Falha ao carregar credenciais da nuvem: {e}")
    ...
    def iniciar_robo_principal(self):
        for widget in self.winfo_children():
            widget.destroy()
        self.title("Dona Francisca / DASHBOARD UNIFICADO (SUPABASE)")
...
class MotorRobo:
    ...
    def tocar_som(self, tipo):
        # Tenta carregar sons customizados da pasta local
        som_path = os.path.join(BASE_DIR, "sounds", f"{tipo}.wav")
        if os.path.exists(som_path):
            ...
            if tipo == "start":
                ...
                # Busca a música nos recursos embutidos (MEIPASS) se estiver compilado,
                # senão busca no diretório pai (PARENT_DIR) ou no diretório base
                if IS_FROZEN:
                    caminho_musica = os.path.join(RESOURCES_DIR, "Escrava-isaura-_fundo_.wav")
                else:
                    caminho_musica = os.path.join(PARENT_DIR, "Escrava-isaura-_fundo_.wav")
                ...
                if os.path.exists(caminho_musica):
                    try:
                        ...
                        self.registrar_falha_caixa_preta(f"tocar_som_sucesso:{caminho_musica}", exc)
    ...
    def salvar_no_banco(self, nome_modulo, curso, professor, status, arquivo_origem=""):
        try:
            self.history_repo.insert_history(nome_modulo, curso, professor, status, arquivo_origem)
            try:
                worker_id = 0
                import inspect
                for frame in inspect.stack():
                    obj = frame.frame.f_locals.get("self")
                    if obj and obj.__class__.__name__ == "FrotaWorker":
            ...
            if self.module_scope == "unico":
                if indice_desejado is not None:
                    ...
                    dados_filtrados = [
                        ...
                        if self._registro_corresponde_filtro(row, filtro)
                    ]
    ...
    def atualizar_botoes_modo(self):
        if hasattr(self, "botoes_preset"):
            for (modo, escopo), botao in self.botoes_preset.items():
                ...
                cor = "#D97706" if selecionado else "#1E293B"
                botao.configure(
                    fg_color=cor,
                    border_color="#1E293B",
                    text_color="#F1F5F9" if selecionado else "#94A3B8",
                )

        if hasattr(self, "botoes_upload") and self.botoes_upload:
            for modo, botao in self.botoes_upload.items():
                selecionado = modo == self.upload_mode
                cor_up = "#1E3A8A" if selecionado else "#1E293B"
                borda_up = "#3B82F6" if selecionado else "#1E293B"
                texto_up = "#FFFFFF" if selecionado else "#94A3B8"
                botao.configure(
                    fg_color=cor_up,
                    border_color=borda_up,
                    text_color=texto_up,
                )

        if hasattr(self, "botoes_escopo") and self.botoes_escopo:
            for escopo, botao in self.botoes_escopo.items():
                selecionado = escopo == self.module_scope
                cor_esc = "#064E3B" if selecionado else "#1E293B"
                borda_esc = "#10B981" if selecionado else "#1E293B"
                texto_esc = "#FFFFFF" if selecionado else "#94A3B8"
                botao.configure(
                    fg_color=cor_esc,
                    border_color=borda_esc,
        ...
        if hasattr(self, "frame_modulo_unico") and self.frame_modulo_unico:
            if self.module_scope == "unico":
                ...
                if hasattr(self, "presets_frame") and self.presets_frame:
                    self.presets_frame.pack_forget()
                    self.presets_frame.pack(fill="x", padx=16, pady=(8, 12))
                if hasattr(self, "lbl_modo_resumo") and self.lbl_modo_resumo:
                    self.lbl_modo_resumo.pack_forget()
    ...
    def selecionar_escopo(self, escopo):
        self.module_scope = escopo
        self.config["module_scope"] = escopo
        self.single_module_target = (
            self.ent_modulo_alvo.get().strip()
            if hasattr(self, "ent_modulo_alvo")
            else self.single_module_target
        )
        self.config["single_module_target"] = self.single_module_target
        try:
            save_config(CONFIG_PATH, self.config)
    ...
    def _abrir_plataformas_worker(self):
        try:
            with self.manual_platform_lock:
                ...
                if ignorou_certificado:
                    self.log(
                        "INFO", "Aviso de certificado ignorado no login manual do site antigo."
                    )
                wait.until(EC.element_to_be_clickable((By.NAME, "logindagestao"))).send_keys(
    ...
    def log(self, tipo, msg):
        ...
        with self.logs_lock:
            self.logs_api.append(
                {
                    "id": f"log_{len(self.logs_api)}_{int(time.time()*1000)}",
                    "timestamp": hora,
                    "level": level_map.get(tipo, "INFO"),
                    "message": msg,
                }
            )
            if len(self.logs_api) > 1000:
                self.logs_api.pop(0)
    ...
    def abrir_leitor_bd(self):
        ...
        _bg = BG_WINDOW if is_dark else "#F1F5F9"
        _card = BG_CARD if is_dark else "#FFFFFF"
        _text = TEXT_LIGHT if is_dark else "#1E293B"
        _muted = TEXT_MUTED if is_dark else "#64748B"
        _input = BG_INPUT if is_dark else "#F8FAFC"
        _border = BORDER_COLOR if is_dark else "#E2E8F0"

        janela = ctk.CTkToplevel(self.root)
        janela.title("🗄️ Histórico de Banco de Dados")
        janela.geometry("900x600")
        janela.configure(fg_color=_bg)
        janela.attributes("-topmost", True)

        ctk.CTkLabel(janela, text="Auditoria Interna (SQLite)", font=("Space Grotesk", 18, "bold"), text_color=_text).pack(
            pady=15
        )
    ...
    def abrir_assistente_ia(self):
        ...
        self.tela_ia.configure(fg_color=_bg)
    ...
    def abrir_dashboard(self):
        ...
        self.janela_dash.configure(fg_color=_bg)
        self.janela_dash.attributes("-topmost", True)
        
        ctk.CTkLabel(
            self.janela_dash, text="Dashboard de Produtividade", font=("Space Grotesk", 18, "bold"), text_color=_text
        ).pack(pady=15)
    ...
    def atualizar_grafico_loop(self):
        if not hasattr(self, "janela_dash") or not self.janela_dash.winfo_exists():
            return
        is_dark = self.config.get("theme", "dark") == "dark"
        _text_color = "white" if is_dark else "black"
        _empty_color = "#334155" if is_dark else "#94A3B8"
    ...
    def abrir_configuracoes(self):
        ...
        tela_cfg.configure(fg_color=_bg)
        
        scroll = ctk.CTkScrollableFrame(tela_cfg, fg_color="transparent")
        scroll.pack(fill="both", expand=True, padx=10, pady=10)

        f_tema = ctk.CTkFrame(scroll, fg_color=_card, border_color=_border, border_width=1, corner_radius=8)
        f_tema.pack(fill="x", pady=6, padx=5)
        ctk.CTkLabel(
            f_tema, text="🎨 Apresentação UI/UX", font=("Space Grotesk", 14, "bold"), text_color=AZUL_PASSO
        ).pack(pady=5)
        cbox_theme = ctk.CTkComboBox(f_tema, values=["dark", "light"], width=200, corner_radius=6)
        cbox_theme.set(self.config.get("theme", "dark"))
        cbox_theme.pack(pady=(5, 10))

        f_busca = ctk.CTkFrame(scroll, fg_color=_card, border_color=_border, border_width=1, corner_radius=8)
        f_busca.pack(fill="x", pady=6, padx=5)
        ctk.CTkLabel(
            f_busca,
        ...
        f_auditoria = ctk.CTkFrame(scroll, fg_color=_card, border_color=_border, border_width=1, corner_radius=8)
        f_auditoria.pack(fill="x", pady=6, padx=5)
        ctk.CTkLabel(
            f_auditoria,
        ...
        f1 = ctk.CTkFrame(scroll, fg_color=_card, border_color=_border, border_width=1, corner_radius=8)
        f1.pack(fill="x", pady=6, padx=5)
        ctk.CTkLabel(f1, text="🌐 Site Antigo", font=("Space Grotesk", 14, "bold"), text_color=_text).pack(pady=5)
        e_ant_url = ctk.CTkEntry(f1, width=400, fg_color=_input, border_color=_border, text_color=_text, corner_radius=6)
        e_ant_url.insert(0, self.config.get("antigo_url", ""))
        e_ant_url.pack(pady=2)
        e_ant_usr = ctk.CTkEntry(f1, width=400, fg_color=_input, border_color=_border, text_color=_text, corner_radius=6)
        e_ant_usr.insert(0, self.config.get("antigo_user", ""))
        e_ant_usr.pack(pady=2)
        e_ant_pwd = ctk.CTkEntry(f1, width=400, show="*", fg_color=_input, border_color=_border, text_color=_text, corner_radius=6)
        e_ant_pwd.insert(0, self.config.get("antigo_pass", ""))
        e_ant_pwd.pack(pady=(2, 10))

        f2 = ctk.CTkFrame(scroll, fg_color=_card, border_color=_border, border_width=1, corner_radius=8)
        f2.pack(fill="x", pady=6, padx=5)
        ctk.CTkLabel(f2, text="🚀 Site Novo", font=("Space Grotesk", 14, "bold"), text_color=_text).pack(pady=5)
        e_nov_url = ctk.CTkEntry(f2, width=400, fg_color=_input, border_color=_border, text_color=_text, corner_radius=6)
        e_nov_url.insert(0, self.config.get("novo_url", ""))
        e_nov_url.pack(pady=2)
        e_nov_usr = ctk.CTkEntry(f2, width=400, fg_color=_input, border_color=_border, text_color=_text, corner_radius=6)
        e_nov_usr.insert(0, self.config.get("novo_user", ""))
        e_nov_usr.pack(pady=2)
        e_nov_pwd = ctk.CTkEntry(f2, width=400, show="*", fg_color=_input, border_color=_border, text_color=_text, corner_radius=6)
        e_nov_pwd.insert(0, self.config.get("novo_pass", ""))
        e_nov_pwd.pack(pady=(2, 10))

        f3 = ctk.CTkFrame(scroll, fg_color=_card, border_color=_border, border_width=1, corner_radius=8)
        f3.pack(fill="x", pady=6, padx=5)
        ctk.CTkLabel(f3, text="🧠 IA (Groq)", font=("Space Grotesk", 14, "bold"), text_color=ROXO_IA).pack(pady=5)
        e_groq = ctk.CTkEntry(f3, width=400, show="*", fg_color=_input, border_color=_border, text_color=_text, corner_radius=6)
        e_groq.insert(0, self.config.get("groq_key", ""))
        e_groq.pack(pady=5)

        f4 = ctk.CTkFrame(scroll, fg_color=_card, border_color=_border, border_width=1, corner_radius=8)
        f4.pack(fill="x", pady=6, padx=5)
        ctk.CTkLabel(
            f4, text="📱 WhatsApp CallMeBot", font=("Space Grotesk", 14, "bold"), text_color="#25D366"
        ).pack(pady=5)
        e_wpp_num = ctk.CTkEntry(f4, width=400, placeholder_text="+55...", fg_color=_input, border_color=_border, text_color=_text, corner_radius=6)
        e_wpp_num.insert(0, self.config.get("whatsapp_number", ""))
        e_wpp_num.pack(pady=2)
        e_wpp_key = ctk.CTkEntry(f4, width=400, show="*", placeholder_text="API Key", fg_color=_input, border_color=_border, text_color=_text, corner_radius=6)
    ...
    def setup_ui(self):
        ...
        self.btn_planilhas.configure(text="PLANILHA DE MAPEAMENTO (ANTIGO → NOVO)")
        ...
        for modo, escopo, texto in [
            ("completo", "todos", "MIGRAR — NOMES MAPEADOS (PLANILHA)"),
        ]:
            botao = ctk.CTkButton(
                presets_frame,
                text=texto,
    ...
    def _setup_ui_stitch(self):
        ...
        bg_window = "#0D131A" if is_dark else "#F1F5F9"
        bg_header = "#0B0F14" if is_dark else "#FFFFFF"
        bg_sidebar = "#0A0F14" if is_dark else "#F8FAFC"
        bg_card = "#131B26" if is_dark else "#FFFFFF"
        bg_input = "#0B0F14" if is_dark else "#F1F5F9"
        
        color_cyan = "#00F2FE" if is_dark else "#0284C7"
        color_border = "#1E293B" if is_dark else "#E2E8F0"
        
        text_light = "#F1F5F9" if is_dark else "#1E293B"
        ...
        header_frame = ctk.CTkFrame(self.root, fg_color=bg_header, height=65, corner_radius=0)
        ...
        bot_icon_frame = ctk.CTkFrame(left_header, fg_color="#083344" if is_dark else "#E0F2FE", border_color="#155E75" if is_dark else "#0284C7", border_width=1, width=38, height=38, corner_radius=8)
        bot_icon_frame.pack(side="left", pady=13)
        bot_icon_frame.pack_propagate(False)
        ctk.CTkLabel(bot_icon_frame, text="🤖", font=("Inter", 20)).pack(expand=True)
        ...
        ctk.CTkLabel(
            title_frame, 
            text="AUTOMAÇÃO DE MÓDULOS MAPEADOS", 
        ...
        self.btn_cfg_header = ctk.CTkButton(
            controls_frame, 
            text="⚙", 
            width=32, 
            height=32, 
            corner_radius=6, 
            fg_color="transparent", 
            hover_color="#1E293B" if is_dark else "#E2E8F0", 
            text_color=text_muted, 
            font=("Inter", 16), 
            command=self.abrir_configuracoes
        )
        self.btn_cfg_header.pack(side="left", padx=2)
        ...
        ctk.CTkButton(
            ...
            text_color=text_muted, 
            font=("Inter", 12), 
            command=lambda: self.root.state("iconic")
        ...
        body_frame = ctk.CTkFrame(self.root, fg_color="transparent")
        body_frame.pack(fill="both", expand=True)

        sidebar_frame = ctk.CTkFrame(body_frame, fg_color=bg_sidebar, width=220, corner_radius=0, border_width=1, border_color=color_border)
        ...
        nav_active_container = ctk.CTkFrame(sidebar_frame, fg_color="#122330" if is_dark else "#E0F2FE", height=42, corner_radius=6)
        nav_active_container.pack(fill="x", padx=10, pady=4)
        nav_active_container.pack_propagate(False)

        indicator = ctk.CTkFrame(nav_active_container, fg_color=color_cyan, width=3, corner_radius=0)
        indicator.pack(side="left", fill="y")

        ctk.CTkLabel(
            nav_active_container, 
            text="   Home / Painel", 
            font=("Space Grotesk", 12, "bold"), 
            text_color=color_cyan
        ).pack(side="left", fill="both", expand=True, anchor="w")

        ctk.CTkFrame(sidebar_frame, fg_color=color_border, height=1).pack(fill="x", padx=15, pady=12)
        ...
        self.btn_db.pack(fill="x", padx=10, pady=2)
        ...
        self.btn_dash.pack(fill="x", padx=10, pady=2)
        ...
        self.btn_ia.pack(fill="x", padx=10, pady=2)

        fila_card = ctk.CTkFrame(sidebar_frame, fg_color=bg_card, border_color=color_border, border_width=1, corner_radius=8)
        fila_card.pack(fill="x", padx=15, pady=15)
        ...
        self.lbl_workers.pack(pady=(0, 12))

        requirements_card = ctk.CTkFrame(sidebar_frame, fg_color=bg_window, border_color=color_border, border_width=1, corner_radius=10)
        requirements_card.pack(side="bottom", fill="x", padx=15, pady=15)

        ctk.CTkLabel(
            requirements_card, 
            text="REQUISITOS / STATUS", 
            font=("Space Grotesk", 9, "bold"), 
            text_color=text_muted
        ).pack(anchor="w", padx=12, pady=(10, 6))

        pandas_status_frame = ctk.CTkFrame(requirements_card, fg_color="transparent", height=18)
        pandas_status_frame.pack(fill="x", padx=12, pady=2)
        ctk.CTkLabel(pandas_status_frame, text="Pandas:", font=("Inter", 10), text_color=text_muted).pack(side="left")
        ctk.CTkLabel(pandas_status_frame, text="✔ OK", font=("Inter", 10, "bold"), text_color=color_green).pack(side="right")

        antigravity_status_frame = ctk.CTkFrame(requirements_card, fg_color="transparent", height=18)
        antigravity_status_frame.pack(fill="x", padx=12, pady=(2, 10))
        ctk.CTkLabel(antigravity_status_frame, text="Antigravity:", font=("Inter", 10), text_color=text_muted).pack(side="left")
        ctk.CTkLabel(antigravity_status_frame, text="✔ Ativo", font=("Inter", 10, "bold"), text_color=color_green).pack(side="right")

        ctk.CTkFrame(requirements_card, fg_color=color_border, height=1).pack(fill="x", pady=2)
        badge_frame = ctk.CTkFrame(requirements_card, fg_color="#083344" if is_dark else "#E0F2FE", corner_radius=4)
        badge_frame.pack(fill="x", padx=12, pady=8)
        ctk.CTkLabel(badge_frame, text="VERSÃO 1.2", font=("Space Grotesk", 9, "bold"), text_color=color_cyan).pack(pady=3)

        workspace_frame = ctk.CTkFrame(body_frame, fg_color=bg_window, corner_radius=0)
        workspace_frame.pack(side="left", fill="both", expand=True, padx=20, pady=20)

        top_workspace = ctk.CTkFrame(workspace_frame, fg_color="transparent")
        top_workspace.pack(fill="x", pady=(0, 15))

        left_top_ws = ctk.CTkFrame(top_workspace, fg_color="transparent")
        left_top_ws.pack(side="left", fill="y")
        ...
        ctk.CTkLabel(
            left_top_ws, 
            text="Gerencie a fila de extração de vídeos e configure o ciclo de migração do robô.", 
            font=("Inter", 12), 
            text_color=text_muted
        ).pack(anchor="w", pady=(2, 0))

        controls_top_right = ctk.CTkFrame(top_workspace, fg_color="transparent")
        controls_top_right.pack(side="right")
        ...
        ctk.CTkButton(
            ...
            height=34, 
            corner_radius=8, 
            fg_color="#121921" if is_dark else "#E2E8F0", 
            hover_color="#1E293B" if is_dark else "#CBD5E1", 
            border_color=color_border, 
            border_width=1, 
            text_color=text_muted, 
            command=self.resetar_progresso_robo
        ).pack(side="left", padx=4)

        delay_val = 1.5
        self.lbl_delay = ctk.CTkLabel(
            controls_top_right, 
            text=f"Delay Ativo: {delay_val:.1f}s", 
            font=("Space Grotesk", 11, "bold"), 
            text_color=color_cyan, 
            fg_color="#020617" if is_dark else "#E2E8F0", 
            corner_radius=8, 
            width=120, 
            height=34
        )
        ...
        grid_container = ctk.CTkFrame(workspace_frame, fg_color="transparent")
        grid_container.pack(fill="both", expand=True)
        
        grid_container.grid_columnconfigure(0, weight=5, uniform="group1")
        grid_container.grid_columnconfigure(1, weight=3, uniform="group1")
        grid_container.grid_columnconfigure(2, weight=4, uniform="group1")
        grid_container.grid_rowconfigure(0, weight=1)

        col1_frame = ctk.CTkFrame(grid_container, fg_color=bg_card, border_width=1, border_color=color_border, corner_radius=12)
        col1_frame.grid(row=0, column=0, sticky="nsew", padx=6, pady=0)

        ctk.CTkLabel(
            col1_frame, 
            text="📊 PLANILHA DE FLUXO / NOMES MAPEADOS", 
            font=("Space Grotesk", 13, "bold"), 
            text_color=text_light
        ).pack(anchor="w", padx=16, pady=(15, 6))

        excel_drop_area = ctk.CTkFrame(col1_frame, fg_color=bg_input, border_color=color_border, border_width=1, corner_radius=8, height=160)
        excel_drop_area.pack(fill="x", padx=16, pady=6)

        ctk.CTkLabel(
            col1_frame, 
            text="📊 MODO DE EXECUÇÃO DO ROBÔ", 
            font=("Space Grotesk", 10, "bold"), 
            text_color=color_cyan
        ).pack(anchor="w", padx=16, pady=(8, 2))
        
        self.cbox_modo_robo = ctk.CTkComboBox(
            col1_frame,
            values=[
                "Mapeamento por Múltiplos Módulos",
                "Ordem Exata (Forma Geral)",
                "Mapeamento por Módulo Único",
                "Migração Separada"
            ],
            width=300,
            command=self.mudar_modo_robo
        )
        self.cbox_modo_robo.set("Mapeamento por Múltiplos Módulos")
        self.cbox_modo_robo.pack(anchor="w", padx=16, pady=(2, 10))
        excel_drop_area.pack_propagate(False)

        ctk.CTkLabel(
            excel_drop_area, 
            text="Clique abaixo para carregar as planilhas", 
            font=("Inter", 11), 
            text_color=text_muted
        ).pack(pady=(25, 8))
        ...
        self.btn_planilhas.pack(fill="x", padx=25, pady=(0, 8))
        ...
        excel_footer_btns = ctk.CTkFrame(col1_frame, fg_color="transparent")
        excel_footer_btns.pack(fill="x", padx=16, pady=4)
        ...
        ctk.CTkButton(
            ...
            border_width=1, 
            corner_radius=6, 
            command=self.baixar_planilha_exemplo
        ...
        self.frame_modulo_unico = ctk.CTkFrame(col1_frame, fg_color=bg_input, border_color=color_border, border_width=1, corner_radius=8)
        self.frame_modulo_unico_title = ctk.CTkLabel(
            self.frame_modulo_unico, 
            text="🎯 ALVO DE MÓDULO ÚNICO", 
            font=("Space Grotesk", 11, "bold"), 
            text_color=color_cyan
        )
        self.frame_modulo_unico_title.pack(anchor="w", padx=12, pady=(8, 2))
        
        self.ent_modulo_alvo = ctk.CTkEntry(
            self.frame_modulo_unico, 
            placeholder_text="Linha ou nome do módulo antigo", 
            height=30, 
            fg_color=bg_card, 
            border_color=color_border, 
            text_color=text_light, 
        ...
        if self.single_module_target:
            self.ent_modulo_alvo.insert(0, self.single_module_target)
        self.ent_modulo_alvo.pack(fill="x", padx=12, pady=(0, 8))
        self.ent_modulo_alvo.bind("<KeyRelease>", self.atualizar_alvo_modulo)
        ...
        presets_frame = ctk.CTkFrame(col1_frame, fg_color="transparent")
        presets_frame.pack(fill="x", padx=16, pady=(8, 12))
        
        self.botoes_preset = {}
        self.btn_planilhas_fluxo = ctk.CTkButton(
            presets_frame, 
            text="MIGRAR — NOMES MAPEADOS (PLANILHA)", 
            font=("Space Grotesk", 12, "bold"), 
            fg_color=bg_card,
            hover_color="#1E293B" if is_dark else "#E2E8F0",
            ...
            height=46,
            corner_radius=8,
            command=lambda: self.selecionar_preset_execucao("completo", "todos")
        )
        self.btn_planilhas_fluxo.pack(fill="x")
        self.botoes_preset[("completo", "todos")] = self.btn_planilhas_fluxo

        # Botão discreto para exibir/ocultar opções avançadas de envio
        self.mostrar_avancado = False
        
        self.avancado_frame = ctk.CTkFrame(
            col1_frame, 
            fg_color=bg_input, 
            border_color=color_border, 
            border_width=1, 
        ...
        def toggle_avancado():
            self.mostrar_avancado = not self.mostrar_avancado
            if self.mostrar_avancado:
                self.avancado_frame.pack(fill="x", padx=16, pady=6)
                self.btn_toggle_avancado.configure(
                    text="⚙ OCULTAR CONFIGURAÇÃO AVANÇADA",
                    text_color=color_cyan
                )
            else:
                self.avancado_frame.pack_forget()
                self.btn_toggle_avancado.configure(
        ...
        self.btn_toggle_avancado = ctk.CTkButton(
            ...
            height=28,
            corner_radius=6,
            command=toggle_avancado
        )
        self.btn_toggle_avancado.pack(fill="x", padx=16, pady=(2, 6))

        # Conteúdo do avançado_frame (Tipo de Envio e Escopo)
        ctk.CTkLabel(
            self.avancado_frame,
            text="TIPO DE ENVIO (MOTO)",
            font=("Space Grotesk", 10, "bold"),
            text_color=color_cyan
        ).pack(anchor="w", padx=12, pady=(10, 4))
        
        upload_sub_frame = ctk.CTkFrame(self.avancado_frame, fg_color="transparent")
        upload_sub_frame.pack(fill="x", padx=12, pady=4)
        ...
        for modo, texto in [
            ...
            self.botoes_upload[modo] = btn

        ctk.CTkLabel(
            self.avancado_frame,
            text="ESCOPO DO MÓDULO",
            font=("Space Grotesk", 10, "bold"),
            text_color=color_green
        ).pack(anchor="w", padx=12, pady=(8, 4))
        
        escopo_sub_frame = ctk.CTkFrame(self.avancado_frame, fg_color="transparent")
        escopo_sub_frame.pack(fill="x", padx=12, pady=(4, 12))
        ...
        col2_frame.grid(row=0, column=1, sticky="nsew", padx=6, pady=0)
        
        col2_frame.grid_columnconfigure(0, weight=1)
        col2_frame.grid_rowconfigure(0, weight=1)
        col2_frame.grid_rowconfigure(1, weight=1)
        col2_frame.grid_rowconfigure(2, weight=1)
        ...
        self.btn_preparar.grid(row=0, column=0, sticky="nsew", pady=(0, 6))
        ...
        self.btn_iniciar.grid(row=1, column=0, sticky="nsew", pady=6)
        ...
        self.btn_abrir_plataformas.grid(row=2, column=0, sticky="nsew", pady=(6, 0))

        col3_frame = ctk.CTkFrame(grid_container, fg_color=bg_card, border_width=1, border_color=color_border, corner_radius=12)
        col3_frame.grid(row=0, column=2, sticky="nsew", padx=6, pady=0)
        ...
        radial_container = ctk.CTkFrame(col3_frame, fg_color="transparent", width=110, height=110)
        radial_container.pack(pady=4)
        radial_container.pack_propagate(False)

        self.canvas_radial = tk.Canvas(radial_container, width=110, height=110, bg=bg_card, highlightthickness=0)
        self.canvas_radial.pack(fill="both", expand=True)

        self.lbl_status_mod_linear = ctk.CTkLabel(
            col3_frame, 
            text="Módulos processados: 0 / 0", 
            font=("Inter", 11, "bold"), 
            text_color=text_light
        )
        self.lbl_status_mod_linear.pack(pady=(10, 2))

        self.progress_bar = ctk.CTkProgressBar(
            col3_frame, 
            progress_color=color_cyan, 
            fg_color="#1E293B" if is_dark else "#E2E8F0", 
            height=6, 
            corner_radius=3
        )
        ...
        metrics_frame = ctk.CTkFrame(col3_frame, fg_color="transparent")
        metrics_frame.pack(fill="x", padx=20, pady=(8, 12))

        self.lbl_eta = ctk.CTkLabel(
            metrics_frame, 
            text="Tempo Estimado (ETA): --:--", 
            font=("Inter", 10), 
            text_color=text_muted
        )
        self.lbl_eta.pack(anchor="w", pady=1)

        self.lbl_pendentes = ctk.CTkLabel(
            metrics_frame, 
            text="Mídias Pendentes: 0", 
            font=("Inter", 10), 
            text_color=text_muted
        )
        self.lbl_pendentes.pack(anchor="w", pady=1)

        self.atualizar_progresso_radial(0)

        console_frame = ctk.CTkFrame(workspace_frame, fg_color=bg_input, border_width=1, border_color=color_border, corner_radius=12)
        console_frame.pack(fill="both", expand=True, pady=(15, 0))

        console_header = ctk.CTkFrame(console_frame, fg_color=bg_sidebar, height=40, corner_radius=0)
        console_header.pack(fill="x", side="top")
        console_header.pack_propagate(False)

        mac_dots = ctk.CTkFrame(console_header, fg_color="transparent")
        mac_dots.pack(side="left", padx=12, pady=12)
        
        ctk.CTkFrame(mac_dots, fg_color="#EF4444", width=10, height=10, corner_radius=5).pack(side="left", padx=2)
        ctk.CTkFrame(mac_dots, fg_color="#F59E0B", width=10, height=10, corner_radius=5).pack(side="left", padx=2)
        ctk.CTkFrame(mac_dots, fg_color="#10B981", width=10, height=10, corner_radius=5).pack(side="left", padx=2)

        ctk.CTkLabel(
            console_header, 
            text="CONSOLE DE SAÍDA DO ROBÔ", 
            font=("Space Grotesk", 10, "bold"), 
            text_color=text_light
        ...
        self.system_status_badge = ctk.CTkLabel(
            ...
            text="SYSTEM STABLE", 
            font=("Inter", 8, "bold"), 
            text_color="#065F46" if not is_dark else color_green, 
            fg_color="#D1FAE5" if not is_dark else "#064e3b", 
            corner_radius=4, 
            width=90, 
            height=20
        )
        self.system_status_badge.pack(side="left", padx=10)
        ...
        ctk.CTkButton(
            ...
            height=22, 
            corner_radius=4, 
            command=self.limpar_logs_console
        ...
        self.lbl_status_mod = ctk.CTkLabel(
            console_frame, 
            text="STATUS: AGUARDANDO PLANILHA...", 
            font=("Space Grotesk", 11, "bold"), 
            text_color=color_cyan, 
            anchor="w"
        )
        self.lbl_status_mod.pack(fill="x", padx=15, pady=3)

        self.txt_log = ctk.CTkTextbox(
            console_frame, 
            font=("Consolas", 12), 
            fg_color="#05080F" if is_dark else "#F8FAFC", 
            border_width=0, 
            corner_radius=0, 
            text_color=text_light
        )
        ...
        self.atualizar_botoes_modo()
        self.log("INFO", "Sistema V1.2 (Dona Francisca Engine) Iniciado com sucesso.")

    def atualizar_progresso_radial(self, percentual):
        if not hasattr(self, "canvas_radial") or not self.canvas_radial:
            return
        is_dark = self.config.get("theme", "dark") == "dark"
        card_bg = "#131B26" if is_dark else "#FFFFFF"
        bg_circle = "#1E293B" if is_dark else "#E2E8F0"
        cyan_color = "#00F2FE" if is_dark else "#0284C7"
        text_color = "#FFFFFF" if is_dark else "#1E293B"
        muted_color = "#94A3B8" if is_dark else "#64748B"

        self.canvas_radial.configure(bg=card_bg)
        self.canvas_radial.delete("all")
        self.canvas_radial.create_oval(8, 8, 102, 102, outline=bg_circle, width=6)
        if percentual > 0:
            angulo = percentual * 3.6
            self.canvas_radial.create_arc(8, 8, 102, 102, start=90, extent=-angulo, outline=cyan_color, width=6, style="arc")
        self.canvas_radial.create_text(55, 47, text=f"{int(percentual)}%", fill=text_color, font=("Space Grotesk", 18, "bold"))
        self.canvas_radial.create_text(55, 68, text="Mapeado", fill=muted_color, font=("Inter", 8, "bold"))

    def baixar_planilha_exemplo(self):
        try:
            exemplo_path = os.path.join(PARENT_DIR, "exemplo_mapeamento.xlsx")
            df = pd.DataFrame(
                columns=[
                    "nome do modulo (antigo)",
                    "nome novo",
                    "curso",
                    "professor",
                ]
            ...
            df.loc[0] = [
                "Saúde da Mulher: Diabetes Gestacional (Concurso da Prefeitura de Ji-Paraná/RO - Enfermeiro)",
                "Diabetes Gestacional na Saúde da Mulher",
                "Enfermagem Especializada",
                "Prof. Wesley",
            ]
            df.to_excel(exemplo_path, index=False)
            print("Planilha exemplo criada")
        except Exception as exc:
            print("Erro ao criar planilha", exc)

    def limpar_logs_console(self):
        self.txt_log.configure(state="normal")
        self.txt_log.delete("1.0", tk.END)
        self.txt_log.configure(state="disabled")
        self.log("INFO", "Console limpo pelo usuário.")

    def resetar_progresso_robo(self):
        with self.progresso_lock if self.progresso_lock else open("/dev/null", "w"):
            self.modulos_concluidos = 0
            self.total_modulos = len(self.lista_de_planilhas[0]["records"]) if self.lista_de_planilhas else 0
            self.stats_contagem = {"Sucesso": 0, "Erro": 0, "Aviso": 0}
        self.progress_bar.set(0)
        self.atualizar_progresso_radial(0)
        self.log("INFO", "Estatísticas de progresso redefinidas pelo usuário.")
    ...
    def preparar_frota(self):
        ...
        self.lbl_workers.configure(text=f"{self.total_modulos} MÓDULOS")
        self.lbl_status_mod_linear.configure(text=f"Módulos processados: 0 / {self.total_modulos}")
        self.progress_bar.set(0)
        self.atualizar_progresso_radial(0)
    ...
    def registrar_conclusao_modulo(self, status="Sucesso"):
        with self.progresso_lock:
            ...
            self.ui_do(lambda: self.atualizar_progresso_radial(pct * 100))
            self.ui_do(
                lambda: self.lbl_status_mod_linear.configure(
                    text=f"Módulos processados: {self.modulos_concluidos} / {self.total_modulos}"
                )
            )
            ...
            # Atualiza ETA
            if self.modulos_concluidos > 0 and self.tempo_inicio_trabalho:
                import time
                elapsed = time.time() - self.tempo_inicio_trabalho
                avg_time = elapsed / self.modulos_concluidos
                remaining_items = self.total_modulos - self.modulos_concluidos
                rem_seconds = int(remaining_items * avg_time)
                eta_str = f"Tempo Estimado (ETA): {rem_seconds//60}m {rem_seconds%60}s"
                self.ui_do(lambda: self.lbl_eta.configure(text=eta_str))
            
            if self.modulos_concluidos == self.total_modulos:
                self.ui_do(lambda: self.lbl_eta.configure(text="Tempo Estimado (ETA): Concluído!", text_color="#10B981"))
            
            self.ui_do(
                lambda: self.lbl_status_mod.configure(
                    text=f"🤖 Dona Francisca: Processados {self.modulos_concluidos}/{self.total_modulos} Módulos"
                )
            )
