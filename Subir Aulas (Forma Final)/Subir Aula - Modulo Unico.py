...
class AppPrincipal(ctk.CTk):
    def __init__(self):
        ...
        self.title("ISAURA V106 — Login | Módulo Único")
    ...
    def iniciar_robo_principal(self):
        for widget in self.winfo_children():
            widget.destroy()
        self.title("ISAURA V106 — MÓDULO ÚNICO (MANUAL)")
...
class MotorRobo:
    ...
    def atualizar_botoes_modo(self):
        ...
        if hasattr(self, "frame_modulo_unico"):
            if self.module_scope == "unico":
                self.frame_modulo_unico.pack(
                    fill="x", padx=20, pady=(0, 10), before=self.actions_frame
                )
            else:
    ...
    def setup_ui(self):
        ...
        for modo, escopo, texto in [
            ("completo", "unico", "MIGRAR MÓDULO COMPLETO — ABA ATIVA"),
            ("videos", "unico", "MIGRAR APENAS VÍDEOS — ABA ATIVA"),
            ("materiais", "unico", "MIGRAR APENAS MATERIAIS — ABA ATIVA"),
        ]:
            botao = ctk.CTkButton(
                presets_frame,
                text=texto,
        ...
        ctk.CTkLabel(
            self.frame_modulo_unico,
            text="Informe o nome de referência. A migração ocorrerá nas abas abertas dos navegadores, sem busca automática.",
            font=("Inter", 12),
            text_color=TEXT_MUTED,
        ).pack(anchor="w", padx=16, pady=(0, 8))
        self.ent_modulo_alvo = ctk.CTkEntry(
            self.frame_modulo_unico,
            placeholder_text="Ex.: Módulo 01 — Introdução ao Python...",
            height=40,
        )
    ...
    def _setup_ui_stitch(self):
        ...
        ctk.CTkLabel(
            ...
            text_color="#06B6D4" if dark_theme else "#0891B2",
        ...
        # db_card.grid(row=0, column=0, sticky="nsew", padx=(0, 10))  # OCULTO no Modo Manual
        ...
        )  # Expandido: Modo Manual
        card_title(modos_frame, "[>]", "O QUE DESEJA MIGRAR?", accent="#06B6D4")
        ctk.CTkLabel(
            modos_frame,
            text="Posicione o Chrome Antigo na aba do módulo de origem e o Chrome Novo na aba de destino. O robô migrará o conteúdo diretamente.",
            font=("Inter", 12),
            text_color=_muted,
            justify="left",
            wraplength=520,
        ...
        ctk.CTkLabel(
            self.frame_modulo_unico,
            text="NOME DO MÓDULO (Para registro no banco)",
            font=("Space Grotesk", 15, "bold"),
            text_color="#06B6D4" if dark_theme else "#0891B2",
