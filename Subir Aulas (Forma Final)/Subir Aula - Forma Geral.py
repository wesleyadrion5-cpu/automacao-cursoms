                    ...
                    try:
                        ...
                        if row:
                            ...
                            if chaves_colunas:
                                ...
                                if valor_ultima.startswith("http://") or valor_ultima.startswith(
                                    "https://"
                                ):
                        ...
                        if link_modulo_direto:
                            self.log(
                                "INFO",
                                f"Navegando direto para o módulo antigo pelo link fornecido: {link_modulo_direto}",
                            )
                            self._abrir_url_com_certificado(
                                link_modulo_direto,
                                f"detalhes do módulo antigo (link direto): {nome_modulo}",
                            )
                            time.sleep(1.5)
                            ...
                            if metodo_busca == "Ordem Exata (Linha 1 = Módulo 1)":
                                if indice_origem < len(tds):
                                    ...
                                    self.log(
                                        "ERRO",
                                        f"O site antigo tem menos módulos do que o seu Excel.",
                                    )
                            else:
                                for td in tds:
...
class AppPrincipal(ctk.CTk):
    def __init__(self):
        ...
        self.title("ISAURA V106 — Login | Lote Geral")
    ...
    def iniciar_robo_principal(self):
        for widget in self.winfo_children():
            widget.destroy()
        self.title("ISAURA V106 — LOTE GERAL (PLANILHA)")
...
class MotorRobo:
            ...
            for (modo, escopo), botao in self.botoes_preset.items():
                ...
                cor = (
                    ...
                    if escopo == "todos"
                        ...
                        AZUL_PASSO
                        if modo == "videos"
                        else "#14B8A6" if modo == "materiais" else VERDE_ACAO
                    )
                )
                hover = (
                    "#D97706"
                    if escopo == "todos"
                        ...
                        "#1D4ED8"
                        if modo == "videos"
                        else "#0F766E" if modo == "materiais" else VERDE_HOVER
                    )
                )
                cor_base = (
                    "#4C3410"
                    if escopo == "todos"
                        ...
                        "#112C66"
                        if modo == "videos"
                        else "#0D4040" if modo == "materiais" else "#113D31"
                    )
                )
                botao.configure(
    ...
    def setup_ui(self):
        ...
        for modo, escopo, texto in [
            ("completo", "todos", "SUBIR TUDO — PLANILHA COMPLETA"),
        ]:
            botao = ctk.CTkButton(
                presets_frame,
                text=texto,
    ...
    def _setup_ui_stitch(self):
        ...
        def action_button(
            parent, text, fg_color, hover_color, border_color, command, state="normal"
        ):
            return ctk.CTkButton(
                parent,
                text=text,
        ...
        ctk.CTkLabel(
            brand,
            text="ISAURA — LOTE GERAL",
            font=("Space Grotesk", 22, "bold"),
            text_color="#F59E0B" if dark_theme else "#D97706",
        ...
        card_title(modos_frame, "[>]", "MODO ATIVO — LOTE VIA PLANILHA", accent="#F59E0B")
        ctk.CTkLabel(
            modos_frame,
            text="Carregue a planilha Excel com todos os módulos a subir. O robô irá percorrer cada linha e migrar completamente.",
            font=("Inter", 12),
            text_color=_muted,
            justify="left",
            wraplength=520,
